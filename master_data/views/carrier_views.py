...
from master_data.serializers import CarrierSerializer, CarrierListRequestSerializer
...
class CarrierFilter(django_filters.FilterSet):
    carrier_name = django_filters.CharFilter(lookup_expr='icontains')
    carrier_scac = django_filters.CharFilter(lookup_expr='icontains')
    ...
    class Meta:
        model = Carrier
        fields = ['carrier_name', 'carrier_scac', 'type', 'transport', 'status', 'created_by', 'created_at']


class CarrierListView(generics.GenericAPIView):
    """
    List all carriers with filtering using POST method
    """
    queryset = Carrier.objects.select_related('transport', 'created_by', 'updated_by').all()
    serializer_class = CarrierSerializer
    permission_classes = [permissions.IsAuthenticated, HasModulePermission]
    module_code = 'carriers'
    ...
    @extend_schema(
        request=CarrierListRequestSerializer,
        responses={200: CarrierSerializer(many=True)},
        tags=['Carrier Management'],
        description="List all active carriers with optional filtering, sorting, and pagination. Use POST method to send filter criteria in the request body.",
        summary="List Carriers (with filtering)",
        operation_id="v1_carrier_list_post"
    )
    def post(self, request, *args, **kwargs):
        ...
        # 1. Apply filters
        filterset = CarrierFilter(clean_data, queryset=queryset)
        ...
        allowed_columns = [f.name for f in Carrier._meta.fields]
        ...
        if search:
            queryset = queryset.filter(
                ...
                Q(carrier_scac__icontains=search) |
...
@extend_schema(tags=['Carrier Management'])
...
class CarrierDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    Retrieve, update or delete a specific carrier
    """
    queryset = Carrier.objects.select_related('transport', 'created_by', 'updated_by').all()
    serializer_class = CarrierSerializer
    permission_classes = [permissions.IsAuthenticated, HasModulePermission]
    ...
    def destroy(self, request, *args, **kwargs) -> Response:
        ...
        return Response(
            {"success": True, "message": "Carrier deleted successfully."},

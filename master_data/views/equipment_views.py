...
from master_data.models import Equipment
from master_data.serializers import EquipmentListRequestSerializer, EquipmentSerializer
...
class EquipmentFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Equipment
        fields = ["code", "type", "status", "created_at"]
...
class EquipmentListView(generics.GenericAPIView):
    ...
    @extend_schema(
        request=EquipmentListRequestSerializer,
        responses={200: EquipmentSerializer(many=True)},
        tags=["Package Management"],
        description="List all active equipment items with filtering, sorting and pagination.",
        summary="List Equipment",
        operation_id="v1_equipment_list_post",
    )
    def post(self, request, *args, **kwargs):
        ...
        filterset = EquipmentFilter(clean_data, queryset=queryset)
        ...
        allowed_columns = [f.name for f in Equipment._meta.fields]

        if sort_column in allowed_columns:
            queryset = queryset.order_by(f"-{sort_column}" if sort_order.lower() == "desc" else sort_column)
...
@extend_schema(tags=["Package Management"])
...
class EquipmentDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        instance = Equipment.all_objects.get(pk=instance.pk)
